// https://github.com/Jocs/promise.git
class Promise {
  ...
  then(
    ...
    let resolveExecutor = (promise2, x, resolve, reject) => {
      ...
      if (x instanceof Promise) {
        ...
        if (x.status === "pending") {
          ...
          // 2.3.2.2 如果/当x被成功时，用相同的值（结果）履行promise。 2.3.2.3 如果/当x被失败时，用相同的错误原因履行promise。;
        ...
        try {
          ...
          if (typeof then === "function") {
            ...
            let resolvePromise = y => {
              ...
              isThenCalled = true
              // 2.3.3.3.1
              // 如果使用值（结果）y调用resolvePromise，运行[[Resolve]]（promise，y）我的解决程序的名字是resolveExecutor,也
              // 就是递归调用。;
              resolveExecutor(promise2, y, resolve, reject)
            }
            let rejectPromise = r => {
            ...
            // 2.3.3.3 如果then是一个函数，则使用x作为此参数调用它，第一个参数resolveExecutor，第二个参数rejectPromise，其中;
          ...
          // 2.3.3.3.4 如果调用then方法抛出异常e， 2.3.3.3.4.1
          // 如果resolvePromise或rejectPromise已经调用了，则忽略它。;
          if (isThenCalled) return
          isThenCalled = true

          //2.3.3.2 如果x.then导致抛出异常e，拒绝promise并用e作为失败原因 2.3.3.3.4.2 否则，以e作为失败原因拒绝promise;
          reject(e)
        }
      } else {
    ...
    if (self.status === "fulfilled") {
      ...
      return (promise2 = new Promise((resolve, reject) => {
        // 2.2.4 在执行上下文堆栈仅包含平台代码之前，不能调用onFulfilled或onRejected。[3.1]。 3.1
        // 这里的“平台代码”是指引擎，环境和primise实现代码。在实践中，这个要求确保onFulfilled和onRejected异步执行，在事件循环开始之后th
        // en被调用，和一个新的堆栈。这可以使用诸如setTimeout或setImmediate之类的“宏任务”机制，或者使用诸如MutationObserver或
        // process.nextTick的“微任务”机制来实现。由于promise实现被认为是经过深思熟虑的平台代码，因此它本身可能包含调用处理程序的任务调度队列或
        // 或称为“trampoline”（可重用的）的处理程序。 让onFulfilled异步执行
        setTimeout(() => {
          try {
